...
public class SensitiveWordUtils {
	 ...
	 *
	 * @param txt        文本
	 * @param replaceStr 替换的字符串，匹配的敏感词以字符逐个替换，如 语句：我爱中国人
	 *                   敏感词：中国人，替换字符串：[屏蔽]，替换结果：我爱[屏蔽]
	 * @param matchType  敏感词匹配规则
	 * @return
	 */
	...
	public static void main(String[] args) {
		...
		result = SensitiveWordUtils.contains(string, SensitiveWordUtils.MaxMatchType);
		System.out.println(result);

		// 获取语句中的敏感词
		Set<String> set = SensitiveWordUtils.getSensitiveWord(string);
		System.out.println("语句中包含敏感词的个数为：" + set.size() + "。包含：" + set);
		set = SensitiveWordUtils.getSensitiveWord(string, SensitiveWordUtils.MaxMatchType);
		System.out.println("语句中包含敏感词的个数为：" + set.size() + "。包含：" + set);

		// 替换语句中的敏感词
		String filterStr = SensitiveWordUtils.replaceSensitiveWord(string, '*');
		System.out.println(filterStr);
		filterStr = SensitiveWordUtils.replaceSensitiveWord(string, '*', SensitiveWordUtils.MaxMatchType);
		System.out.println(filterStr);

		String filterStr2 = SensitiveWordUtils.replaceSensitiveWord(string, "[*敏感词*]");
		System.out.println(filterStr2);
		filterStr2 = SensitiveWordUtils.replaceSensitiveWord(string, "[*敏感词*]", SensitiveWordUtils.MaxMatchType);
		System.out.println(filterStr2);
	}
